...
async def message_detector(dut):
    current_notes = dut.note_out.value.integer
    current_wave = dut.wave_out.value.integer
    last_state = dut.state.value
    while 1:
        await FallingEdge(dut.clk)
        if dut.state.value != last_state:
            if dut.state.value:
                dut._log.info('ACTIVE')
            else:
                dut._log.info('DONE')
        if current_notes != dut.note_out.value.integer:
            dut._log.info(f"Recieved {dut.second_last_byte.value} {dut.last_byte.value} {dut.current_byte.value}. Velocity[{dut.last_byte.value.integer}] = {dut.velocity_out}")
        if current_wave != dut.wave_out.value.integer:
...
async def test_a(dut):
    ...
    await FallingEdge(dut.clk)
    dut.rst.value = 0
    #cocotb.start_soon(message_detector(dut))
    await Timer(10, 'ns')
    await FallingEdge(dut.clk)
    for _ in range(20):
        current_notes = dut.note_out.value.integer
    ...
    while current_notes:
        if current_notes%2:
            input = [[1,0,0,0,0,0,0,0], bits(number), [0,0,0,0,0,1,1,1]]
            dut._log.info(f"Sending Off: {input[1]}")
            for byte in input:
                dut.data_in.value = 0
                await Timer(32000, 'ns')
